...
func LoadCompactNeedleMap(file *os.File, newIdxOffset uint64, oldNm *NeedleMap) (*NeedleMap, error) {
	if newIdxOffset == 0 {
		oldNm = nil
	}
	nm := NewCompactNeedleMap(file, oldNm)
	return doLoading(file, nm, newIdxOffset)
}

func doLoading(file *os.File, nm *NeedleMap, newIdxOffset uint64) (*NeedleMap, error) {
	e := idx.WalkIndexFile(file, newIdxOffset, func(key NeedleId, offset Offset, size Size) error {
